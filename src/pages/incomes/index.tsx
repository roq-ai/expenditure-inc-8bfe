...
type ColumnType = ColumnDef<IncomeInterface, unknown>;
...
export function IncomeListPage(props: IncomeListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<IncomeInterface>>(
    () => `/incomes?params=${JSON.stringify(params)}`,
    fetcher,
  );
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/incomes/view/${record.id}`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              mr={2}
              padding="0rem 8px"
